...
- **扩写中文摘要** — 结构化扩写后可选生成中文摘要,只展示不传下游,帮你看懂最终 prompt 说了啥
- **本地永久缓存** — 生成结果下载到 `library/`,原 URL 过期也能访问
- **作品归档** — 右侧抽屉展示历史作品(按项目隔离)
- **★ 全局画廊** — 在归档里点 ☆ 收藏的作品,跨项目聚合到 `/gallery` 页面,支持点击播放视频
- **移动端触屏适配** — 长按节点弹菜单、双指缩放、底部撤销/重做、多选模式;触屏笔电自动识别
